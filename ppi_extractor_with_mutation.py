...
def negation_position(b,sen,entity_dict,rel_dict):
	...
	l = sen.split(" ")
	for i in l:
		if i=="no" or i=="not" or i=="neither" or i=="nor" or i=="cannot":
			print(i)
			m = {}
			m[sen.index(i)] = 'neg'
			for key,value in b.items():
...
def mutation_detector(b,sen,id):
	mut = mutation_finder(id)
	mut.append("mut")
	mut.append("Mut")
	for i in mut:
		if i in sen:
...
#     data_for_training = pickle.load(f)



xmldoc_AIMED = minidom.parse('/Users/fanision/Desktop/Thesis_Research/corpora/AIMED-unified-format.xml')
	...
	try:
		...
		for sentence in itemlist:
			if sentence.attributes["id"].value.split(".")[2]==id:
				sen = sentence.attributes['text'].value
				article_id = sentence.attributes['id'].value.split(".")[1][1:]
		itemlist = xmldoc_AIMED.getElementsByTagName('interaction')
		interaction_record=[]
		for interaction in itemlist:
			if interaction.attributes["e1"].value.split(".")[2]==id:
				if interaction.attributes["e1"].value<=interaction.attributes["e2"].value:
					...
					if b not in interaction_record:
		...
		if count>=2:
			...
			############new
			####################new: detect whether there is a mutaiton mention in the sentence
			#none is 0, there is, it's 1
			print(article_id)
			#b=mutation_detector(b,sen,article_id)
		...
		escapped_list.append(s)
	

xmldoc_AIMED = minidom.parse('/Users/fanision/Desktop/Thesis_Research/corpora/bioinfer-unified-format.xml')
...
print(escapped_list)
with open('axuilary_corpus_training.pickle', 'wb') as handle:
